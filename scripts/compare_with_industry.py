...
def assess_performance(bench_name: str, actual_ns: float) -> Tuple[str, str, str]:
    ...
    Returns: (status, emoji, explanation)
    """
    ...
    if not target_info:
        return "unknown", "❓", "No industry baseline available"
    ...
    # Assess
    if actual_ns <= goal:
        return "excellent", "✅", f"Meets goal ({format_time(goal)})"
    elif actual_ns <= target:
        return "good", "✓", f"Within target ({format_time(target)})"
    elif actual_ns <= industry_max:
        return "acceptable", "⚠️", f"Competitive with industry max ({format_time(industry_max)})"
    else:
        return "poor", "❌", f"Exceeds industry max ({format_time(industry_max)})"
...
- ✅ **Excellent**: Meets or exceeds goal performance
- ✓ **Good**: Within target performance
- ⚠️ **Acceptable**: Competitive with industry maximum
- ❌ **Poor**: Exceeds industry maximum (needs optimization)
- ❓ **Unknown**: No industry baseline available
...
"""
    ...
    for category, benchmarks in sorted(categories.items()):
        ...
        for bench_name, actual_ns in sorted(benchmarks):
            status, emoji, explanation = assess_performance(bench_name, actual_ns)
            status_counts[status] += 1

            report += f"| {bench_name} | {format_time(actual_ns)} | {emoji} | {explanation} |\n"
    ...
    report += f"- ✅ Excellent: {status_counts['excellent']} ({100*status_counts['excellent']/total:.1f}%)\n"
    report += f"- ✓ Good: {status_counts['good']} ({100*status_counts['good']/total:.1f}%)\n"
    report += f"- ⚠️ Acceptable: {status_counts['acceptable']} ({100*status_counts['acceptable']/total:.1f}%)\n"
    report += f"- ❌ Poor: {status_counts['poor']} ({100*status_counts['poor']/total:.1f}%)\n"
    report += f"- ❓ Unknown: {status_counts['unknown']} ({100*status_counts['unknown']/total:.1f}%)\n"
    report += "\n"
    ...
    print("="*60)
    print(f"✅ Excellent: {status_counts['excellent']}")
    print(f"✓  Good:      {status_counts['good']}")
    print(f"⚠️  Acceptable: {status_counts['acceptable']}")
    print(f"❌ Poor:      {status_counts['poor']}")
    print(f"❓ Unknown:   {status_counts['unknown']}")
    print("="*60)

    if poor_benchmarks:
        print("\n⚠️  WARNING: Some benchmarks exceed industry standards!")
        return 1
