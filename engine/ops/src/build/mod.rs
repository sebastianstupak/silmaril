...
//! This module contains the core build logic without any CLI or UI dependencies.
...
///
/// Reports progress via the provided [`ProgressSink`](crate::ProgressSink).
#[allow(clippy::too_many_arguments)]
pub fn build_all_platforms(
    ...
    progress.on_start("build", platform_names.len());
    ...
    for (i, name) in platform_names.iter().enumerate() {
        progress.on_step("build", i, &format!("building {name}"));
        ...
        if let Err(e) = result {
            if platform.is_experimental() {
                ...
                progress.on_done("build", false);
                return Err(e);
            }
        }
    }

    progress.on_done("build", true);
    Ok(())
}
