//! List installed modules in a silmaril game project.

use anyhow::Result;
use std::fs;
use std::path::Path;

use super::parse_cargo_lock_version;

/// List all modules recorded in `game.toml`.
///
/// Reads `[modules]` from `game.toml`, resolves versions from `Cargo.lock`
/// where applicable, and logs a formatted table.
pub fn list_modules(project_root: &Path) -> Result<()> {
    ...
    let req_w = modules
        .iter()
        .map(|(_, v)| {
            extract_field(v, "version")
                .or_else(|| extract_field(v, "tag"))
                .or_else(|| extract_field(v, "ref"))
                .or_else(|| extract_field(v, "path"))
                .unwrap_or_default()
                .len()
        })
        .max()
...
fn parse_modules_section(content: &str) -> Vec<(String, String)> {
    ...
    for line in content.lines() {
        ...
        if trimmed.starts_with('[') && trimmed != "[modules]" {
            in_modules = false;
            continue;
        }
