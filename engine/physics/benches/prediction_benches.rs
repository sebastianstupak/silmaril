...
fn bench_input_retrieval(c: &mut Criterion) {
    ...
    group.bench_function("remove_before", |b| {
        ...
        b.iter(|| {
            buffer_clone.remove_before(black_box(25));
...
fn bench_reconciliation(c: &mut Criterion) {
    ...
    group.bench_function("reconcile_no_error", |b| {
        let mut physics_clone = physics.clone();
        let mut system_clone = PredictionSystem::new();
        system_clone.start_prediction(entity, physics_id, Vec3::ZERO, Quat::IDENTITY, Vec3::ZERO);

        b.iter(|| {
            // Reconcile with exact match (no error)
            system_clone.reconcile(
                black_box(55),
                black_box(Vec3::ZERO),
    ...
    group.bench_function("reconcile_small_error", |b| {
        ...
        for i in 0..60 {
            system_clone.add_input_and_predict(
                i * 16,
...
fn bench_error_smoothing(c: &mut Criterion) {
    ...
    world.add(entity, engine_math::Transform::from_position(Vec3::ZERO));

    let mut system = PredictionSystem::new();
    system.start_prediction(entity, 1, Vec3::ZERO, Quat::IDENTITY, Vec3::ZERO);

    // Create error
    if let Some(state) = &mut system.predicted_state {
