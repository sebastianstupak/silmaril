...
fn test_prediction_with_physics_integration() {
    ...
    system.add_input_and_predict(1000, Vec3::new(1.0, 0.0, 0.0), false, 0.016, &mut physics);
...
fn test_reconciliation_with_small_error() {
    ...
    system.reconcile(2, Vec3::new(0.1, 0.0, 0.0), Quat::IDENTITY, Vec3::ZERO, &mut physics);
...
fn test_error_smoothing() {
    ...
    // Create prediction error - need to start prediction first
    let config = PhysicsConfig::default();
    let mut physics = PhysicsWorld::new(config);
    ...
    // Create error by manually modifying predicted position
    // Since predicted_state is private, we'll simulate error through reconciliation
    system.add_input_and_predict(0, Vec3::new(1.0, 0.0, 0.0), false, 0.016, &mut physics);
    physics.step(0.016);
...
fn test_prediction_sequence_monotonic() {
    ...
    let _seq1 = system.current_sequence();
